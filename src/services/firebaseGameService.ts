...
export function leaveGame(gameId: string, userId: string): Promise<void> {
  return new Promise((resolve, reject) => {
    // Сначала добавляем событие о выходе
    const gameRef = ref(db, `games/${gameId}`);
    
    onValue(gameRef, (snapshot) => {
      const game = snapshot.val();
      if (game && game.players && game.players[userId]) {
        const playerName = game.players[userId].name;
        const playerNumber = game.players[userId].number;
        
        // Добавляем ход о выходе
        addGameMove(
          gameId,
          userId,
          playerName,
          playerNumber,
          'leave',
          `Игрок ${playerNumber} покинул игру`,
          { leftAt: Date.now() }
        ).then(() => {
          // Затем удаляем игрока
          const playerRef = ref(db, `games/${gameId}/players/${userId}`);
        ...
        // Если игрок не найден, просто удаляем
        const playerRef = ref(db, `games/${gameId}/players/${userId}`);
        remove(playerRef).then(() => resolve()).catch(reject);
      }
    }, { onlyOnce: true });
  });
}
